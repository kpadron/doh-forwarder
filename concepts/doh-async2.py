...
def main():
	...
	for upstream in upstreams:
		...
		conns.append(loop.run_until_complete(upstream_connect(upstream)))
...
async def upstream_connect(url):
	"""
	...
	conn = vibora.client.Session(headers=headers)
...
async def upstream_forward(url, data, conn):
	...
	while True:
		response = await conn.post(url, body=data, decode=False)
		if response.status_code == 200:
			return response.content

		logging.warning('%s (%d) IN %s, OUT %s' % (url, response.status_code, data, response.content))
