...
def main():
	...
	for upstream in upstreams:
		...
		cache = dns.resolver.LRUCache(10000)
		resolver = DohResolver(upstream)
		resolver.connect(headers, 4)
		resolvers.append(resolver)

	# Setup UDP server
	logging.info('Starting UDP server listening on: %s#%d' % (host, port))
	udp_listen = loop.create_datagram_endpoint(lambda: UdpDohProtocol(resolvers[0]), local_addr = (host, port))
...
class DohResolver:
	...
	def __init__(self, upstream, cache=None):
		self.upstream = upstream
		self.conn = None
		self.cache = cache

	def connect(self, headers, timeout):
		"""
		Prepare connection object corresponding to upstream server.
		"""
		...
		# self.conn = aiohttp.ClientSession(connector=connector, timeout=timeout, headers=headers)
		self.conn = aiohttp.ClientSession(headers=headers)
	...
	async def resolve(self, query):
		...
		while True:
			try:
				...
				async with self.conn.post(self.upstream, data=query) as http:
					if http.status == 200:
					...
					logging.warning('%s (%d): IN %s, OUT %s' % (self.upstream, http.status, query, await http.read()))

			# Log client connection errors (aiohttp should attempt to reconnect on next request)
			except aiohttp.ClientConnectionError as exc:
				logging.error('Client error, %s: %s' % (self.upstream, exc))

			# Log request timeout errors
			except asyncio.TimeoutError as exc:
				logging.error('Timeout error, %s: %s' % (self.upstream, exc))
				return b''
